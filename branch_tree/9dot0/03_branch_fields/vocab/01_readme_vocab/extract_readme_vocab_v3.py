#!/usr/bin/env python3
"""
README vocab extractor v3 for Repo.9Dot0 vocab.branch.

Purpose:
- Read only explicitly listed README.md source units.
- Preserve raw occurrences.
- Filter common function words.
- Rescue short structural core tokens such as C, m, t, p, ?, S, A, Path, Core.
...
SOURCES: list[SourceUnit] = [
    SourceUnit("SeungeFlow/9Dot0", "REPO_9DOT0", "main", "7aba83cd9b3ae0461e9dafd7a38a63b509c8d8eb", "README.md", "Repo.9Dot0 entry surface"),
    SourceUnit("SeungeFlow/9Dot0", "REPO_9DOT0", "main", "7aba83cd9b3ae0461e9dafd7a38a63b509c8d8eb", "03_branch_fields/market/README.md", "branch.market surface"),
    SourceUnit("SeungeFlow/9Dot0", "REPO_9DOT0", "main", "7aba83cd9b3ae0461e9dafd7a38a63b509c8d8eb", "03_branch_fields/sohosa/README.md", "branch.sohosa surface"),
    SourceUnit("SeungeFlow/9Dot0", "REPO_9DOT0", "main", "7aba83cd9b3ae0461e9dafd7a38a63b509c8d8eb", "03_branch_fields/history/README.md", "branch.history surface"),
    SourceUnit("SeungeFlow/9Dot0", "REPO_9DOT0", "main", "7aba83cd9b3ae0461e9dafd7a38a63b509c8d8eb", "03_branch_fields/vocab/README.md", "branch.vocab surface"),
...
TOKEN_RE = re.compile(
    r"C=\(m,t,p,\?\)|C=tp|9dot0|Ctp24|Ctp|S[1-4]|S[₁₂₃₄]|Y_Branch|vocab\.branch|README\.md|Seed\.Base|Raw_URL|[A-Za-z][A-Za-z0-9_./+-]*|[가-힣]{1,}|[\u4E00-\u9FFF]{1,}|[0-9]+|[?]"
)
...
ENGLISH_COMMON = {
    ...
    "markdown", "file", "files", "readme", "md", "role", "status", "first", "current",
    "later", "future", "under", "over", "only", "all", "whole", "new",
}

CORE_TOKENS = {
    "C", "m", "t", "p", "?", "S", "A", "Path", "Core",
    "C=(m,t,p,?)", "C=tp", "Ctp", "Ctp24",
    "S1", "S2", "S3", "S4", "S₁", "S₂", "S₃", "S₄",
}
...
STRUCTURAL_ALLOWLIST = {
    "AI", "Ctp", "Ctp24", "S1", "S2", "S3", "S4", "S₁", "S₂", "S₃", "S₄",
    "Y_Branch", "SeungeFlow", "9dot0", "dot", "diff", "difference", "orbit", "COG",
    "boundary", "field", "source", "identity", "guard", "relation", "matrix", "swap",
    "branch", "vocab.branch", "vocab", "README.md", "Raw", "URL", "Raw_URL", "context",
    "window", "Seed", "Seed.Base", "DB", "reader", "directory", "repo", "runtime",
    "surface", "entry", "observer", "target", "criterion", "Path", "Core",
...
def has_core_context(line: str) -> bool:
    return any(pattern in line for pattern in CORE_CONTEXT_PATTERNS)


def is_core_token(token: str, line: str) -> bool:
    if token in CORE_TOKENS:
        return True
    if token in {"C", "m", "t", "p", "?", "S", "A"} and has_core_context(line):
        return True
    return False


def is_common(token: str, line: str) -> bool:
    if is_core_token(token, line):
        return False
    lower = token.lower()
...
def classify_token(token: str, line: str) -> str:
    if is_core_token(token, line):
        return "core_token_candidate"
    if is_common(token, line):
        return "demoted_common"
    if token in STRUCTURAL_ALLOWLIST:
        return "structural_trigger_candidate"
    return "filtered_candidate"


def structural_score(token: str, count: int, source_count: int, statuses: set[str]) -> int:
    score = 0
    if "core_token_candidate" in statuses:
        score += 12
    if token in STRUCTURAL_ALLOWLIST:
        score += 10
...
def main() -> int:
    ...
    md_report = out_dir / "readme_vocab_scan_core_filtered_preview_pass14.md"
    csv_report = out_dir / "readme_vocab_scan_core_filtered_preview_pass14.csv"
    ...
    term_statuses: dict[str, set[str]] = defaultdict(set)
    ...
    for source in SOURCES:
        ...
        for line_no, line in enumerate(text.splitlines(), start=1):
            for token in tokenize(line):
                status = classify_token(token, line)
                raw_counter[token] += 1
                source_key = f"{source.repo_label}@{source.commit}:{source.path}"
                term_sources[token].add(source_key)
                term_statuses[token].add(status)

                if status == "demoted_common":
                    demoted_counter[token] += 1
                elif status == "core_token_candidate":
    ...
    for token, count in filtered_counter.items():
        ...
        score = structural_score(token, count, source_count, term_statuses[token])
        structural_rank.append((score, count, source_count, token))
    structural_rank.sort(key=lambda x: (-x[0], -x[1], x[3]))

    core_rank = []
    for token, count in core_counter.items():
        source_count = len(term_sources[token])
        score = structural_score(token, count, source_count, term_statuses[token])
        core_rank.append((score, count, source_count, token))
    core_rank.sort(key=lambda x: (-x[0], -x[1], x[3]))
    ...
    with md_report.open("w", encoding="utf-8") as f:
        f.write("# README vocab core-filtered preview pass14\n\n")
        f.write("## status\n\n")
        f.write("This is a core-token-aware filtered preview generated from explicitly listed README sources only.\n\n")
        ...
        f.write(f"- core_terms: {len(core_counter)}\n")
        ...
        f.write("| rank | term | score | count | source_count | statuses |\n")
        f.write("|---:|---|---:|---:|---:|---|\n")
        for idx, (score, count, source_count, token) in enumerate(structural_rank[:180], start=1):
            statuses = ",".join(sorted(term_statuses[token]))
            f.write(f"| {idx} | `{token}` | {score} | {count} | {source_count} | {statuses} |\n")

        f.write("\n## top core token candidates\n\n")
        f.write("| rank | term | score | count | source_count |\n")
        f.write("|---:|---|---:|---:|---:|\n")
        for idx, (score, count, source_count, token) in enumerate(core_rank[:80], start=1):
    ...
    print(f"core_terms={len(core_counter)}")
